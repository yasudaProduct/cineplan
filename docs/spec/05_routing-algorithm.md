...
**期待値は検算済み・確定**（待ち時間等の定義は `spec/10_dp-implementation.md` §4 を正とする）。
...
（本期待値と実装が食い違ったら、まず定義（spec/10 §4）と本表の手計算を再確認し、それでも仕様側が誤りなら本ドキュメントを先に直す。）

## 9. 非採用とした代替案（記録）

ADR へ移管済み（docs 再編 2026-07-29）。MIP/外部ソルバ・貪欲法の不採用、および同一作品重複を DP 状態で厳密管理しない判断（§3 の棄却方式）は ADR-0006 を、リクエスト毎の経路検索 API 呼出の不採用は ADR-0005 を参照。
