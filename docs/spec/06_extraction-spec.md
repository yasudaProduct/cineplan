...
| V5 | 同一 (movieTitle, startTime, screenName) の重複。**screenName 未公開（空文字）の行はスキップ**（ADR-0024） | `DUPLICATE_ROW` |
...
- **V5 の screenName 未公開館スキップ（2026-09-06 追加・ADR-0024）**: スクリーン名を公開しない劇場（`screenName` が空文字。例: 大阪ステーションシネマ）は、**同一作品を同時刻に別スクリーンで並行上映していても区別する情報が無く**、正当な2行が重複に見える（実例: 2026-09-06 の「水曜どうでしょう祭UNITE2026 ライブ・ビューイング」17:30×2枠）。V7 と同じ理由でこれらの行を重複判定から外し、その劇場群では **V2 が主防御**になる。DB は UNIQUE キーでこれらを同一の上映として扱うため、書込前に畳む（`docs/spec/09` §4.1）。
