...
| 0022 | ST の取込ディスパッチ Cron を一時停止（0021 の一部更新） | Accepted |
| 0023 | 抽出のスキーマ NG を行単位で許容し、日別失敗で run 全体を落とさない | Accepted |
| 0024 | スクリーン名非公開館の同時刻・同作品を V5 の対象外にし、書込前に UNIQUE キーで畳む（0020 の carve-out を V5 へ拡張） | Accepted |
