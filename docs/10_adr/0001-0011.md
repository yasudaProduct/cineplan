...
## Decision
抽出の既定プロバイダを **Google Gemini Flash（無料ティア）** とする。呼び出しはプロバイダ抽象化した抽出クライアント越しに行い、`provider`（`gemini` | `workers-ai` | `anthropic`）と `model` を設定値（wrangler var）で切替可能にする。JSON 構造化出力は Gemini の `responseMimeType=application/json` + `responseSchema`（docs/06 §3 の zod を JSON Schema 化）で担保する。ローカル開発は既存 Ollama でも試せる。
