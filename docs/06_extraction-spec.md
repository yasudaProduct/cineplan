...
- **孤児run の掃除（保険）**: 上記の修正後も、真にハングしたケースへの保険として、`db/ingest-runs.ts` の `reapStaleRuns()` が `queued`/`fetching`/`extracting` のまま20分以上動いていない run を `extraction_failed` に確定する。`/admin` ダッシュボード読込時に呼ばれる（新規 Cron は追加しない）。
