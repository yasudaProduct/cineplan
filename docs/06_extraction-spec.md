...
### プロンプト text_v2 骨子（日単位分割・ADR-0017）
...
- **日単位分割（ADR-0017）の予算感**: 呼出数は劇場あたり 1+日数（週間ページで8回/取込。現行4館 ≒25回/日・10館全て週間ページでも ≒90回/日）で、無料枠 1,500 req/日（ADR-0011）に対し十分小さい。入力（ページ全文）を日数分再送するため in トークンは 約8万/run（21k字ページ×8呼出）だが、500万 in-tok/日 閾値にも遠い。Queue が直列（`max_concurrency=1`）のため RPM/TPM は分割前と変わらない。
- 最適化の優先順（必要になってから着手）:
