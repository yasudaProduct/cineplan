...
// ステータス・属性 enum（docs/spec/02 用語集の正規定義）
...
// 複数日取得（ADR-0019・docs/spec/06 §2.2）。1ページに1日分しか掲載しないサイト向け。
...
// 1セッションで取得するページ数の絶対上限（docs/spec/08 §0・§3）。
// DB の CHECK（migration 0005）・zod・fetch 実装の三箇所で守る。
// 引き上げには docs/spec/08 §6 の変更手続き（ADR + ポリシー改定）が必要。
export const MAX_FETCH_DAYS = 10

// 公開 API レスポンスの Theater（docs/spec/04 components.schemas.Theater）。
