// 共有プラン（P5-1。F-12/F-13・docs/spec/03 §3.6・docs/spec/09）。
// plan_json は Plan（API レスポンス形式）のスナップショット。再計算しない（F-13）。

export interface SharedPlanRow {
  id: string
  plan_json: string
  expires_at: string // UTC ISO（アプリ側で採番時に付与。docs/spec/09 §3）
}
...
// ISO の 'T' と文字列比較互換がないため、SQL 内比較を避けて JS で比較する（docs/spec/09 §3）。
